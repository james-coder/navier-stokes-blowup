...
| 12. Mean corrections, §8; [ActualCycleParameters.signed_request][parameters] and [GlobalSlowProfiles.exists_repaired_order][slow] | Updated residual, auxiliary means and five radial debts → compact mean velocity and pressure increments | Zero torus mean for fast-time inversion; reserved interior radial patch | Torus small-divisor inversion, five-moment inverse and pressure reconstruction; all discarded-mode/tail errors must be carried forward. |
...
1. `NaturalProfile.exists_profileFamily` obtains an existential `Λ₀` from `AxisReference.exists_positive_scaled_profiles`. Turning that into a certified numerical threshold requires effective analytic coefficient bounds, an operator inverse/contraction bound and error budgets on the actual pressure datum's complex neighborhood.
